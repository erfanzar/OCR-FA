from .utils import (
    ...
    recognition_models,
    detection_models,
    all_lang_list as all_language_list,
    MODULE_PATH,
    arabic_lang_list as arabic_language_list,
    BASE_PATH,
)
import torch
import os
...
class OCR:
    ...
    def __init__(
        ...
        if recog_network in ["standard"] + [
            model for model in recognition_models["gen1"]] + [model for model in recognition_models["gen2"]]:
            if recog_network in [model for model in recognition_models["gen1"]]:
                ...
                self.model_lang = model["model_script"]
            else:  # auto-transform_ua
                unknown_lang = set(language_list) - set(all_language_list)
                if unknown_lang != set():
                    raise ValueError(unknown_lang, "is not supported")
                # choose recognition model
                if language_list == ["en"]:
                    self.setModelLanguage("english", language_list, ["en"], "['en']")
                    model = recognition_models["gen2"]["english_g2"]
                    recog_network = "generation2"
                elif set(language_list) & set(arabic_language_list):
                    self.setModelLanguage("arabic", language_list, arabic_language_list + ["en"],
                                          "['ar','fa','ur','ug','en']")
            ...
            if recognizer:
                if os.path.isfile(model_path) is False:
                    ...
                    assert calculate_md5(model_path) == model["md5sum"], "READER ERROR!"
                    ...
                    assert calculate_md5(model_path) == model["md5sum"], "READER ERROR!"
                    LOGGER.info("Download complete")
            self.setLanguageList(language_list, model)
            ...
            global img_height  # if custom model, save this variable. (from *.yaml)
            if recog_config["imgH"]:
                img_height = recog_config["imgH"]
            available_lang = recog_config["language_list"]
            self.setModelLanguage(recog_network, language_list, available_lang, str(available_lang))
            ...
            self.setLanguageList(language_list, recog_config)
    ...
    def getDetectorPath(self, detect_network):
        if detect_network in self.support_detection_network:
            ...
            if self.detect_network == "craft":
                from .detectors import get_detector, get_textbox
            elif self.detect_network in ["dbnet18"]:
                from .detectors import get_detector_db as get_detector, get_textbox_db as get_textbox
            else:
    ...
    def setModelLanguage(self, language, language_list, list_lang, list_lang_string):
        self.model_lang = language
        if set(language_list) - set(list_lang) != set():
            if language == "ch_tra" or language == "ch_sim":
                language = "chinese"
            raise ValueError(
                language.capitalize() + " is only compatible with English, try language_list=" + list_lang_string)
    ...
    def setLanguageList(self, language_list, model):
        self.lang_char = [
            "آ", "ا", "ب", "پ", "ت", "ث",
            "ج", "چ", "ح", "خ", "د", "ذ",
            "ر", "ز", "ژ", "س", "ش", "ص",
    ...
    def recognize(
        ...
        if ((batch_size == 1) or (self.device == "cpu")) and not rotation_info:
            ...
            if rotation_info and image_list:
                ...
                max_width = max(max_width, img_height)
    ...
    def readtext(
        ...
        horizontal_list, free_list = self.transform_ua(
            img,
            min_size=min_size,
            text_threshold=text_threshold,
            low_text=low_text,
        ...
        # get the 1st result from hor & free list as self.transform_ua returns a list of depth 3
    ...
    def readtext_batched(
        ...
        horizontal_list_agg, free_list_agg = self.transform_ua(
            img,
            min_size=min_size,
            text_threshold=text_threshold,
            low_text=low_text,
    ...
    def __repr__(self):
        detector_str = self.detector.__str__().replace("\n", "\n\t")
        return self.__class__.__name__ + (
            f"("
            f"\n\tdetection_models={pformat(self.detection_models)}"
            f"\n\tdetect_network=OCR-FA"
